...
export async function GET(request: NextRequest) {
  ...
  let dbQuery = supabase
    .from('jobs')
    .select(`*, employer:employer_profiles(id, company_name, company_logo_url, industry, verified)`,
      { count: 'exact' })
    .eq('status', 'active')
  ...
  return NextResponse.json({
    data: (data || []).map(job => ({ ...job, is_saved: savedJobIds.includes(job.id) })),
    count: count || 0,
    page,
...
export async function POST(request: NextRequest) {
  ...
  const { title, description, requirements, responsibilities,
    country, city, type, category, salary_min, salary_max,
    salary_currency, experience_years_min, application_deadline } = body
